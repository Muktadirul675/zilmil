  ...
  <div class="w-full lg:w-[70%] mx-auto">
    ...
    <div v-else-if="product" class="flex flex-col md:flex-row gap-4">
      ...
      <div class="w-full md:w-[70%]">
        ...
        <div class="relative overflow-hidden border border-gray-300 lg:rounded-lg">
      ...
      <div class="w-full md:w-[30%] flex flex-col gap-2 lg:gap-4">
        ...
        <div v-if="product.variants?.length" class="flex p-2 lg:p-0 flex-wrap gap-2">
          <div v-for="variant in product.variants" :key="variant.id"
            class="px-3 py-1 text-white transition-all rounded cursor-pointer bg-red-500 hover:bg-red-600" @click="selectedVariant = variant.id">
        ...
        <div class="flex gap-2 w-full p-2 lg:p-0">
          <div class="flex border border-gray-300 rounded-md w-full overflow-hidden">
            <button class="px-3 py-1 text-2xl cursor-pointer" @click="quantity = Math.max(1, quantity - 1)">-</button>
            <input type="number" class="w-full text-center outline-none appearance-none" v-model.number="quantity"
              min="1" />
            <button class="px-3 py-1 text-2xl cursor-pointer" @click="quantity++">+</button>
          </div>
          <button
            class="bg-red-500 cursor-pointer hover:bg-red-600 transition-all text-white px-4 py-2 rounded-md w-full">
        ...
        <div class="my-2 px-2 lg:px-0">
          ...
          <template v-for="(cat, index) in product.categories" :key="cat.id">
            ...
            <span v-if="index !== product.categories.length - 1" class="text-gray-500">
              ,
            </span>
          </template>
        </div>
    ...
    <!-- Description -->
    <div v-if="product" class="lg:mt-6 p-3 rounded-md">
      <div class="text-lg font-semibold mb-2">Description</div>
      <p v-html="product.description" class="text-gray-700 whitespace-pre-line">
      </p>
    </div>
