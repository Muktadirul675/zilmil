                        ...
                        <div className="my-2">
                            <div className="bg-white min-w-[200px] flex-grow border rounded flex flex-col">
                                {categories.map((c, index) => {
                                    if (c.id) {
                                        return <Link key={index} onMouseEnter={() => setActiveSub(index)} href={`/categories/${c.id}`} className="hover:bg-red-500 hover:text-whitetext-lg h-[52px] flex justify-start items-center px-3 border-b">
                                            {c.name}
                                        </Link>
                                    }
                                    return <div key={index} onMouseEnter={() => setActiveSub(index)} className="hover:bg-red-500 hover:text-white text-lg h-[52px] flex justify-start items-center px-3 border-b">
                                        {c.name}
                            ...
                            {categories.map((c, index) => {
                                if (c.subs && c.subs.length) {
                                    return <div style={{ marginTop: `${52 * index}px` }} key={index} className={`bg-white font-normal border flex flex-col w-full rounded ${activeSub !== index ? 'hidden' : 'block'}`}>
                                        {c.subs?.map((s) => {
                                            return <Link key={s.id} href={`/categories/${s.id}`} className="hover:bg-red-500 hover:text-white p-3 border-b last:border-b-0">
                                                {s.name}
                                            </Link>
                                        })}
                                    </div>
